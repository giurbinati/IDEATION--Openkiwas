...
namespace Drupal\opendatatable\Form;
...
class opendatatable extends FormBase
{
  ...
  private const STATE_KEY_LAST_SHEET = 'opendatatable.last_sheet';

  public function getFormId(): string
  {
    return 'opendatatable_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array
  {
    ...
    if ($can_manage) {
      ...
      $form['xlsx'] = [
        ...
        '#upload_location' => 'public://opendatatable/',
        '#upload_validators' => [
    ...
    $form['#attached']['library'][] = 'opendatatable/ideation_table';
      ...
      if ($file) {
        ...
        $form['table_wrapper']['description'] = [
          ...
          'text' => [
            '#markup' => '
      <p>
        The catalogue identifies relevant open data sources, including Earth Observation (EO) satellite data,
        as well as environmental and socio-economic. These sources include local and remote sensors
        (such as satellite imagery, weather stations, and water quality monitoring devices), research outputs
        (such as census reports, land use maps, and economic indicators). Each resource is evaluated for quality,
        update frequency, accessibility, and any applicable legal constraints.
      </p>
    ',
          ],
        ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void
  {
    ...
    try {
      ...
      \Drupal::logger('opendatatable')->error('Errore XLSX: @e', ['@e' => $e->getMessage()]);
  ...
  private function buildTableFromFile(File $file, int $sheetIndex = 0): array
  {
    ...
    \Drupal::logger('opendatatable')->notice('Loading XLSX: @path sheet=@sheet', [
      '@path' => $realPath,
      '@sheet' => $sheetIndex,
    ]);
    ...
    \Drupal::logger('opendatatable')->notice('Detected range: rows=@r cols=@c', [
      '@r' => $highestRow,
    ...
    \Drupal::logger('opendatatable')->notice('Headers found: @h', [
      '@h' => implode(' | ', $headers),
    ]);
        ...
        if ($colIndex === $firstColIndex) {
          ...
          $table_row[] = [
            'data' => [
              '#type' => 'link',
              '#title' => $value,
              '#url' => \Drupal\Core\Url::fromRoute('opendatatable.detail', [
                'row' => $rowNum - 1,
              ]),
            ],
          ];
    ...
    \Drupal::logger('opendatatable')->notice('Rendering table: header=@h rows_total=@rt rows_page=@rp sort=@s order=@o', [
      '@h' => count($header_row),
      '@rt' => $total,
      '@rp' => count($rows_page),
      '@s' => $sort,
