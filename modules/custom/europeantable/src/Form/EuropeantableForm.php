...
namespace Drupal\europeantable\Form;
...
class EuropeantableForm extends FormBase
{

  private const STATE_KEY_LAST_FID = 'europeantable.last_fid';
  private const STATE_KEY_LAST_SHEET = 'europeantable.last_sheet';

  public function getFormId(): string
  {
    return 'europeantable_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array
  {
    ...
    if ($can_manage) {
      ...
      $form['xlsx'] = [
        ...
        '#upload_location' => 'public://europeantable/',
        '#upload_validators' => [
    ...
    $form['#attached']['library'][] = 'europeantable/ideation_table';
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void
  {
    ...
    try {
      ...
      \Drupal::logger('europeantable')->error('Errore XLSX: @e', ['@e' => $e->getMessage()]);
  ...
  private function buildTableFromFile(File $file, int $sheetIndex = 0): array
  {
    ...
    \Drupal::logger('europeantable')->notice('Loading XLSX: @path sheet=@sheet', [
      '@path' => $realPath,
      '@sheet' => $sheetIndex,
    ]);
    ...
    \Drupal::logger('europeantable')->notice('Detected range: rows=@r cols=@c', [
      '@r' => $highestRow,
    ...
    \Drupal::logger('europeantable')->notice('Headers found: @h', [
      '@h' => implode(' | ', $headers),
    ]);
        ...
        if ($colIndex === $firstColIndex) {
          ...
          $table_row[] = [
            'data' => [
              '#type' => 'link',
              '#title' => $value,
              '#url' => \Drupal\Core\Url::fromRoute('europeantable.detail', [
                'row' => $rowNum - 1,
              ]),
            ],
          ];
    ...
    \Drupal::logger('europeantable')->notice('Rendering table: header=@h rows_total=@rt rows_page=@rp sort=@s order=@o', [
      '@h' => count($header_row),
      '@rt' => $total,
      '@rp' => count($rows_page),
      '@s' => $sort,
