...
namespace Drupal\metereologicaltable\Form;
...
class MetereologicaltableForm extends FormBase
{
  ...
  {
    return 'metereologicaltable_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array
  {
    ...
    if ($can_manage) {
      ...
      $form['xlsx'] = [
        ...
        '#upload_location' => 'public://metereologicaltable/',
        '#upload_validators' => [
    ...
    $form['#attached']['library'][] = 'metereologicaltable/ideation_table';
      ...
      if ($file) {
        ...
        $form['table_wrapper']['description'] = [
          ...
          'text' => [
            '#markup' => '
      <p>
        It focuses on data relevant to the inland water digital twin, including weather forecasts, climate data, and hydrological data and services.
      </p>
    ',
          ],
        ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void
  {
    ...
    try {
      ...
      \Drupal::logger('metereologicaltable')->error('Errore XLSX: @e', ['@e' => $e->getMessage()]);
  ...
  private function buildTableFromFile(File $file, int $sheetIndex = 0): array
  {
    ...
    \Drupal::logger('metereologicaltable')->notice('Loading XLSX: @path sheet=@sheet', [
      '@path' => $realPath,
      '@sheet' => $sheetIndex,
    ]);
    ...
    \Drupal::logger('metereologicaltable')->notice('Detected range: rows=@r cols=@c', [
      '@r' => $highestRow,
    ...
    \Drupal::logger('metereologicaltable')->notice('Headers found: @h', [
      '@h' => implode(' | ', $headers),
    ]);
        ...
        if ($colIndex === $firstColIndex) {
          ...
          $table_row[] = [
            'data' => [
              '#type' => 'link',
              '#title' => $value,
              '#url' => \Drupal\Core\Url::fromRoute('metereologicaltable.detail', [
                'row' => $rowNum - 1,
              ]),
            ],
          ];
    ...
    \Drupal::logger('metereologicaltable')->notice('Rendering table: header=@h rows_total=@rt rows_page=@rp sort=@s order=@o', [
      '@h' => count($header_row),
      '@rt' => $total,
      '@rp' => count($rows_page),
      '@s' => $sort,
