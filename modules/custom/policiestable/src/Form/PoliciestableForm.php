...
namespace Drupal\policiestable\Form;
...
class PoliciestableForm extends FormBase
{

  private const STATE_KEY_LAST_FID = 'policiestable.last_fid';
  private const STATE_KEY_LAST_SHEET = 'policiestable.last_sheet';

  public function getFormId(): string
  {
    return 'policiestable_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array
  {
    ...
    if ($can_manage) {
      ...
      $form['xlsx'] = [
        ...
        '#upload_location' => 'public://policiestable/',
        '#upload_validators' => [
    ...
    $form['#attached']['library'][] = 'policiestable/ideation_table';
      ...
      if ($file) {
        ...
        $form['table_wrapper']['description'] = [
          ...
          'text' => [
            '#markup' => '
      <p>
        This catalogue collects and analyses existing and upcoming EU policies that include inland waters, as well as policies from the United Kingdom and Switzerland as these countries are part of the IDEATION project but not in the EU.
      </p>
    ',
          ],
        ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void
  {
    ...
    try {
      ...
      \Drupal::logger('policiestable')->error('Errore XLSX: @e', ['@e' => $e->getMessage()]);
  ...
  private function buildTableFromFile(File $file, int $sheetIndex = 0): array
  {
    ...
    \Drupal::logger('policiestable')->notice('Loading XLSX: @path sheet=@sheet', [
      '@path' => $realPath,
      '@sheet' => $sheetIndex,
    ]);
    ...
    \Drupal::logger('policiestable')->notice('Detected range: rows=@r cols=@c', [
      '@r' => $highestRow,
    ...
    \Drupal::logger('policiestable')->notice('Headers found: @h', [
      '@h' => implode(' | ', $headers),
    ]);
        ...
        if ($colIndex === $firstColIndex) {
          ...
          $table_row[] = [
            'data' => [
              '#type' => 'link',
              '#title' => $value,
              '#url' => \Drupal\Core\Url::fromRoute('policiestable.detail', [
                'row' => $rowNum - 1,
              ]),
            ],
          ];
    ...
    \Drupal::logger('policiestable')->notice('Rendering table: header=@h rows_total=@rt rows_page=@rp sort=@s order=@o', [
      '@h' => count($header_row),
      '@rt' => $total,
      '@rp' => count($rows_page),
      '@s' => $sort,
